...
<body>
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-md-4">
                <form class="loginArea" method="POST" action="{{ route('password.update') }}">
					@csrf
                    <!-- Password Reset Token -->
            <input type="hidden" name="token" value="{{ $request->route('token') }}">
					<div class="inputArea" id="loginArea">
						...
						<input type="email" id="email" name="email" placeholder="Enter Email" value="{{ old('email', $request->email) }}" required readonly>

						<div class="loginInputLabel">Password</div>
						<input type="password" id="password" name="password" required>

						<div class="loginInputLabel">Confirm Password</div>
						<input type="password" id="password" name="password_confirmation" required>
                        <button class="loginBtn btnRed" id="loginBtn" type="submit">Reset Password</button>
					</div>
				</form>
			</div>
		</div>
